...
char *C_HL_extensions[] = {".c",  ".h", ".cpp", ".hpp",
                           ".cc", NULL}; // Array must be terminated with NULL.
    ...
    if (prev_sep) {
      ...
      for (j = 0; keywords[j]; j++) {
        ...
        if (!strncmp(&row->render[i], keywords[j], kw_len) &&
          ...
          memset(&row->highlight[i], kw_2 ? HL_KEYWORD_2 : HL_KEYWORD_1,
                 kw_len);
          i += kw_len;
          break;
        }
...
void editorPaste() {
  ...
  if (ec.cursor_y == ec.num_rows)
    editorInsertRow(ec.cursor_y, ec.copied_char_buffer,
                    strlen(ec.copied_char_buffer));
  else
    editorRowAppendString(&ec.row[ec.cursor_y], ec.copied_char_buffer,
                          strlen(ec.copied_char_buffer));
...
void editorInsertNewline() {
  ...
  if (ec.cursor_x == 0) {
    ...
    editorInsertRow(ec.cursor_y + 1, &row->chars[ec.cursor_x],
...
void revert(Action *action) {
  ...
  case InsertChar: {
    ec.cursor_x = action->cpos_x;
    ec.cursor_y = action->cpos_y;
    editorRowDelString(&ec.row[ec.cursor_y], ec.cursor_x,
                       strlen(action->string));
...
void editorScroll() {
  ...
  // checks if the cursor is past the bottom of the visible window, and contains
  // slightly more complicated arithmetic because ec.row_offset refers to what's
  // at the top of the screen, and we have to get ec.screen_rows involved to
  // talk about what's at the bottom of the screen.
...
void printHelp() {
  ...
  printf("-e | --extension <file_extension> <file_name>   Specify the file "
         "extension\n");
  printf("-t | --use-tabs [file_name]                     Use tabs instead of "
         "spaces\n");

...
typedef struct editor_row {
    ...
    unsigned char *highlight; // This will tell you if a character is part of a
...
char *C_HL_extensions[]      = {".c",
                                ".h",
                                ".cpp",
                                ".hpp",
                                ".cc",
                                NULL}; // Array must be terminated with NULL.
    ...
    if (c == '\x1b') {
        ...
        if (seq[0] == '[') {
            if (seq[1] >= '0' && seq[1] <= '9') {
                ...
                if (seq[2] == '~') {
                    switch (seq[1]) {
                    // Home and End keys may be sent in many ways depending on
                    // the OS
...
void consoleBufferOpen() {
    // Switch to another terminal buffer in order to be able to restore state at
    // exit by calling consoleBufferClose().
...
void editorUpdateSyntax(editor_row *row) {
    ...
    int prev_sep =
        1; // True (1) if the previous char is a separator, false otherwise.
    int in_string =
        0; // If != 0, inside a string. We also keep track if it's ' or "
    int in_comment =
        (row->idx > 0 &&
         ec.row[row->idx - 1]
             .hl_open_comment); // This is ONLY used on ML comments.
    ...
    while (i < row->render_size) {
        ...
        if (ec.syntax->flags & HL_HIGHLIGHT_STRINGS) {
            if (in_string) {
                ...
                // (\), and there’s at least one more character in that line
                // that comes after the backslash, then we highlight the
                // character that comes after the backslash with HL_STRING and
                // consume it. We increment i by 2 to consume both characters at
                // once.
...
void editorInsertRow(int at, char *s, size_t line_len) {
    ...
    memmove(
        &ec.row[at + 1], &ec.row[at], sizeof(editor_row) * (ec.num_rows - at));
    ...
    ec.row[at].chars =
        malloc(line_len + 1); // We want to add terminator char '\0' at the end
...
void editorCopy(bool printStatus) {
    ...
    if (printStatus)
        editorSetStatusMessage("Content copied");
}
...
void editorPaste() {
    ...
    if (ec.cursor_y == ec.num_rows)
        editorInsertRow(
            ec.cursor_y, ec.copied_char_buffer, strlen(ec.copied_char_buffer));
    else
        editorRowAppendString(&ec.row[ec.cursor_y],
                              ec.copied_char_buffer,
...
void editorInsertNewline() {
    ...
    if (ec.cursor_x == 0) {
        ...
        editorInsertRow(
            ec.cursor_y + 1, &row->chars[ec.cursor_x], row->size - ec.cursor_x);
...
void editorSave() {
    ...
    free(buf);
    editorSetStatusMessage("Cant's save file. Error occurred: %s",
                           strerror(errno));
}
...
void editorSearchCallback(char *query, int key) {
    ...
    if (saved_hightlight) {
        memcpy(ec.row[saved_highlight_line].highlight,
               saved_hightlight,
    ...
    for (i = 0; i < ec.num_rows; i++) {
        ...
        if (match) {
            ...
            memset(
                &row->highlight[match - row->render], HL_MATCH, strlen(query));
            break;
        }
    }
}
...
void editorSearch() {
    ...
    char *query = editorPrompt("Search: %s (Use ESC / Enter / Arrows)",
    ...
    case InsertChar: {
        ...
        if (ec.cursor_y < ec.num_rows) {
            editorRowInsertString(
                &ec.row[ec.cursor_y], ec.cursor_x, action->string);
...
void revert(Action *action) {
    ...
    case InsertChar: {
        ...
        ec.cursor_y = action->cpos_y;
        editorRowDelString(
            &ec.row[ec.cursor_y], ec.cursor_x, strlen(action->string));
...
void addAction(Action *action) {
    ...
    if (list->head == NULL) {
        ...
        AListNode *clear_from =
            list->current == NULL ? list->head : list->current->next;
...
// If last action is InsertChar operation and the current action is also
// InsertChar Instead of creating new action, this function concats the char to
// the stored string in last action provided the current action does append at
// the end of the row
bool concatWithLastAction(ActionType t, char *str) {
    if (t == InsertChar && ACTIONS_LIST_MAX_SIZE && ec.actions->current &&
        ec.actions->current == ec.actions->tail &&
        ec.actions->current->action->t == t &&
        ec.actions->current->action->cpos_y == ec.cursor_y &&
        (int)(ec.actions->current->action->cpos_x +
              strlen(ec.actions->current->action->string)) == ec.cursor_x) {
        int c = *(str);
        editorInsertChar(c);
...
void editorScroll() {
    ...
    if (ec.cursor_y < ec.num_rows)
        ec.render_x =
            editorRowCursorXToRenderX(&ec.row[ec.cursor_y], ec.cursor_x);
    // The first if statement checks if the cursor is above the visible window,
    // and if so, scrolls up to where the cursor is. The second if statement
    // checks if the cursor is past the bottom of the visible window, and
    // contains slightly more complicated arithmetic because ec.row_offset
    // refers to what's at the top of the screen, and we have to get
    // ec.screen_rows involved to talk about what's at the bottom of the screen.
...
void editorDrawMessageBar(struct a_buf *ab) {
    ...
    // Here the original code did something smart which I removed because I
    // didn't want to implement time()
    if (msg_len)
    ...
    for (y = 0; y < ec.screen_rows; y++) {
        ...
        if (file_row >= ec.num_rows) {
            ...
            for (j = 0; j < len; j++) {
                ...
                if (iscntrl(c[j])) {
                    char sym = (c[j] <= 26) ? '@' + c[j] : '?';
                    abufAppend(ab, "\x1b[0;7m", 6);
                    abufAppend(ab, &sym, 1);
                    abufAppend(ab, "\x1b[0;0m", 6);
                    if (current_color != -1) {
                        char buf[16] = "\x1b[0;";
                        itoa(current_color, &buf[4], 10);
...
void editorProcessKeypress() {
    ...
    case ARROW_RIGHT:
        ...
        break;
    case PAGE_UP:
    case PAGE_DOWN: { // You can't declare variables directly inside a switch
                      // statement.
        if (c == PAGE_UP)
...
void printHelp() {
    ...
    printf("Ctrl-Q        Exit\n");
    printf("Ctrl-S        Save\n");
    printf("Ctrl-F        Search. Esc, enter and arrows to interact once "
           "searching\n");
    ...
    printf("-v | --version                                  Prints the version "
           "of tte\n");
    printf("-e | --extension <file_extension> <file_name>   Specify the file "
           "extension\n");
    printf(
        "-t | --use-tabs [file_name]                     Use tabs instead of "
        "spaces\n");

    printf("\n\nFor now, usage of ISO 8859-1 is recommended.\n");
}

// > 0 if editor should load a file, 0 otherwise and -1 if the program should
// exit
    ...
    if (argc > 1) {
        if (strncmp("-h", argv[1], 2) == 0 ||
            ...
            return -1;
                   ...
                   strncmp("--version", argv[1], 9) == 0) {
            printf("tte - version %s\n", TTE_VERSION);
            return -1;
        } else if (strncmp("-t", argv[1], 2) == 0 ||
                   strncmp("--use-tabs", argv[1], 10) == 0) {
            ec.use_tabs = 1;
            return argc > 2 ? 2 : 0;
        } else if (strncmp("-e", argv[1], 2) == 0 ||
                   strncmp("--extension", argv[1], 11) == 0) {
            if (argc > 3) {
                ...
                printf(
                    "[ERROR] You must specify an extension and a file name\n");
                return -1;
            }
        }
    }
...
int main(int argc, char *argv[]) {
    ...
    editorSetStatusMessage(
        " Ctrl-Q to quit | Ctrl-S to save | (tte -h | --help for more info)");

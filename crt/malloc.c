...
static t_alloc *
_setAlloc(t_alloc *head, t_alloc *prev, t_alloc *next, unsigned int size) {
...
static t_alloc *_getAlloc(t_zone *zone, size_t size, void *end) {
    ...
    while (head->next) {
        if ((char *)head->next >
            ...
            ptr = _setAlloc((t_alloc *)(head->payload + head->size),
                            head,
                            head->next,
                            size);
            head->next->prev = ptr;
...
static char *
_soft_realloc(t_zone *zone, t_alloc *alloc, size_t size, size_t zonesize) {
    char *next;
